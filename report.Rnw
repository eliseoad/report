...
% FUCTION EDITED bY: Eliseo Acevedo and José A. Bartolomei
%----------------------------------------SPECIAL FUNCTIONS-----------------------------------------%
...
\item Adults who reported not beign physically active are not athletically active had a
...
\item Adults who don't make exercise had \Sexpr{Funct4Interp(type = "or", tabl.exerany2, "est")} \Sexpr{set_MeasureL} when compared with those who do.

\end{itemize}

\begin{figure}[H]
\caption{\Sexpr{set_Measure} among athletically active/inactive \Sexpr{set_Population}, 
\Sexpr{set_Years}}
\label{fig:exe}
...
latex(tabl.exerany2,
      ...
      table.env = T,
      here = T,
      caption = paste(set_Measure, "among", set_Population, "by physical exercise status,",
                      set_Years, sep = " "),
      label = "tab:exe")
@
...
\item A tendency in the three BMI categories evaluated was observed, 
in which those who are neither overweight nor obese had a  
\ifthenelse{
...
\item Figure \ref{fig:smoke} shows that \Sexpr{set_MeasureL} among smokers was \ifthenelse{
\Sexpr{floor(as.numeric(t.asma.current.rfsmok3.df[1,2])*10000)} > 
\Sexpr{floor(as.numeric(t.asma.current.rfsmok3.df[2,2])*10000)}}{lower}{higher} than non-smokers.

\item Adults who smoke had \Sexpr{Funct4Interp(type = "or", tabl.rfsmok3, "est")} of having \Sexpr{set_MeasureL} when compared with those who don't. \Sexpr{Funct4Interp(type = "or", tabl.rfsmok3, "sig")}
...
\Sexpr{set_Years}}
\label{fig:anychronc}
<<cvdcrhd, fig.align='center', echo=FALSE, fig.height=4, fig.width=5, dev='jpeg'>>=
...
<<PRvsUSAchild, fig.align='center', echo=FALSE, fig.height=4, fig.width=7, dev='jpeg'>>=
...
\Sexpr{set_Years}}
\label{fig:hregionch}
<<p.chregion, fig.align='center', echo=FALSE, fig.height=3.5, fig.width=7, dev='jpeg'>>=
...
\begin{figure}[H]
\caption{\Sexpr{set_Measure} among children by annual income of the interviewee, 
\Sexpr{set_Years}}
\label{fig:incomgch}
...
\item \Sexpr{set_Measure} among children was higher where interviewees described their marital status as 
\Sexpr{tolower(rownames(tabl.cmarital2)[which.max(t.current.child.marital2[1:6,2] *10000)])}.

\item Children of households where the interviewees responded their marital status as 
\Sexpr{Funct4Interp(type = "or", tabl.cmarital2, "had")} \Sexpr{set_MeasureL} when compared with those whose parents or gurdians where maried. (See Table \ref{tab:marital2ch}).

\end{itemize}

\begin{figure}[H]
\caption{\Sexpr{set_Measure} among children by marital status of the interviewee, 
\Sexpr{set_Years}}
...
latex(tabl.cmarital2,
      ...
      table.env = T,
      here = T,
      caption = paste(set_Measure, "among children by marital status of the interviewee,",
                      set_Years, 
                      sep = " "),
...
\item Children of households where interviewees responded their
...
\item Children of households where interviewees responded being a
\Sexpr{Funct4Interp(type = "or", tabl.cemplrec2, "being")} Data shown in Table \ref{tab:employch}.

\end{itemize}

\begin{figure}[H]
\caption{\Sexpr{set_Measure} among children by employment status of the interviewee, 
\Sexpr{set_Years}}
\label{fig:employch}
